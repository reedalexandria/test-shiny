...
ggplot(pumpkins, aes(x = day, y = price)) +
  ...
  labs(title = "Pumpkin Price by Day",
       x = "Day",
       y = "Price ($)")
...
new_pumpkins %>% 
  ...
  geom_point(size = 1.6, col = "orange") +
  labs(title = "Pumpkin Price by Day",
       x = "Day",
       y = "Price ($)")
...
new_pumpkins_mean <- new_pumpkins |>
  group_by(month) |>
...
ggplot(new_pumpkins_mean, aes(x = month, y = mean_price)) +
  geom_bar(stat = 'identity', fill = "orange") +
  labs(title = "Average Pumpkin Price by Month",
       x = "Day",
       y = "Average Price ($)")
...
# Find the correlation between the month and the price
...
plot_results %>%
  ggplot(mapping = aes(x = package_integer, y = price)) +
   ...
   xlab("package") +
   labs(title = "Pumpkin Price by Package",
       x = "Package",
       y = "Price ($)")
